# InsquareSeoBundle
Maintained fork of [leogout/SeoBundle](https://github.com/leogout/SeoBundle), adapted for modern Symfony/PHP projects.
This fork includes:
- Symfony `7.3` compatibility
- PHP `8.3` compatibility
- extensible custom attributes for `link`/`meta` tags (for example `hreflang`)
...
This is an example for the `basic` generator:
**In your resource:**
```php
...
These interfaces extend _simpler interfaces_ that you can implement instead, or additionally.
...
If the built-in generators don't suit your needs, InsquareSeoBundle provides a way to create your own SEO generators.
...
## Custom attributes (issue #30)

You can pass custom attributes directly to `addLink()` or `addMeta()`.
Tag content and attribute values are HTML-escaped by default for safer rendering.
Example inside your own SEO generator (class extending `AbstractSeoGenerator`):

```php
$this->tagBuilder->addLink(
    'alternate_pl',
    'https://site.com/page-pl',
    'alternate',
    null,
    null,
    ['hreflang' => 'pl']
...
```html
<link href="https://site.com/page-pl" rel="alternate" hreflang="pl" />
```
